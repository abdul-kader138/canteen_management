...
class Products extends MY_Controller
{
    ...
    function getProducts($warehouse_id = NULL)
    {
        ...
        if ($warehouse_id) {
            $this->datatables
            ->select($this->db->dbprefix('products') . ".id as productid, {$this->db->dbprefix('products')}.image as image, {$this->db->dbprefix('products')}.code as code, {$this->db->dbprefix('products')}.name as name, {$this->db->dbprefix('categories')}.name as cname, cost as cost, price as price, COALESCE(discount_amount, 0) as quantity, {$this->db->dbprefix('units')}.code as unit, wp.rack as rack, type", FALSE)
            ->from('products');
            ...
            $this->datatables
                ->select($this->db->dbprefix('products') . ".id as productid, {$this->db->dbprefix('products')}.image as image, {$this->db->dbprefix('products')}.code as code, {$this->db->dbprefix('products')}.name as name, {$this->db->dbprefix('categories')}.name as cname, cost as cost, price as price, COALESCE(discount_amount, 0) as quantity, {$this->db->dbprefix('units')}.code as unit, '' as rack,type", FALSE)
                ->from('products')
